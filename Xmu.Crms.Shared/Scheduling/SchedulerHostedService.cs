                    ...
                    var realParams =
                        dbset
                            ...
                            .Where(e => eventAttribute.WhereColumns
                                .Select(col =>
                                    col.StartsWith("!")
                                        ? Tuple.Create(col.Substring(1), true)
                                        : Tuple.Create(col, false))
                                .Select(ci => ci.Item2 ^ Convert.ToBoolean(type.GetProperties()
                                                  .Single(ppt => ppt.Name == ci.Item1).PropertyInfo.GetValue(e)))
                                .All(b => b))
                            .Select(e =>

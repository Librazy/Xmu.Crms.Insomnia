            ...
            services
                ...
                .AddJwtBearer(
                    options =>
                    {
                        ...
                        {
                            Debug.WriteLine(eventContext);
                            return Task.CompletedTask;
                        };
                        options.Events.OnTokenValidated += async eventContext =>
                        {
                            await Task.Run(() => eventContext.Principal.AddIdentity(new ClaimsIdentity(
                                new[]
                                {
                                    new Claim(
                                        ClaimTypes.Role,
                                        (eventContext.SecurityToken as JwtSecurityToken ??
                                         throw new InvalidOperationException()).Claims
                                        .Single(c => c.Type == "type").Value.ToUpper()
                                    )
                                }
                            )));
                        };
